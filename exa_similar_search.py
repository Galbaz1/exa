...
def search_and_summarize(question, num_results=5):
    ...
    responses = exa.find_similar_and_contents(
        question, 
    ...
    for result in responses.results:
        if result.text:
            ...
            print(colored(f"\nURL: {result.url}", 'blue'))
            print(colored(f"Relevance Score: {result.score}", 'blue'))

    print(colored("Preparing data for OpenAI GPT...", 'green'))
...
while True:
    question = input(colored("Provide URL: ", 'yellow'))
